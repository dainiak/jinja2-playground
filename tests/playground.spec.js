...
async function expectOutput(page, expected, options = {}) {
	await expect
		.poll(async () => getEditorValue(page, "output"), {
			timeout: 30_000,
			...options,
		})
		.toBe(expected);
...
test("env option checkboxes exist and default to unchecked", async ({
	page,
}) => {
	for (const id of [
		"opt-trim-blocks",
...
test("lstrip_blocks strips leading whitespace before block tags", async ({
	page,
}) => {
...
test("old sharing link without envOptions defaults checkboxes to off", async ({
	page,
}) => {
	// Build a hash using only {templateString, variablesString} via the page's pako
	const hash = await page.evaluate(() => {
		const obj = {
			templateString: "Hello, {{ x }}!",
			variablesString: '{"x": "test"}',
		};
